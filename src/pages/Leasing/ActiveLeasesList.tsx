...
export default function ActiveLeasesList() {
  const { user, isHeadOffice } = useAuth();
  const [items, setItems] = useState<ActiveLeaseItem[]>([]);
  ...
  // Detail drawer
  const [selectedItem, setSelectedItem] = useState<ActiveLeaseItem | null>(null);
  ...
  const fetchItems = async () => {
    setLoading(true);
    try {
      const res = await apiClient.get("/leasing-applications/active", {
        params: {
      ...
      setToast({ type: "error", message: "Failed to load active leases." });
    } finally {
      setLoading(false);
    }
  };
        ...
        render: (item: ActiveLeaseItem) => (
          <div className="flex justify-end">
            <button
              ...
              className="p-1 px-2.5 bg-gray-50 hover:bg-emerald-50 text-gray-500 hover:text-emerald-600 dark:bg-gray-900 dark:hover:bg-emerald-500/10 border border-gray-150 dark:border-gray-800 rounded-lg transition-all flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider"
              title="View Details"
            >
  ...
  return (
    <div className="relative pb-20">
      <PageMeta
        title="Active Leases | Asipiya Leasing"
        description="View and manage active, arrears, and settled leases"
      />
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 mt-4">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">
            Monitor disbursed leases, balances, arrears, and repayment schedules
          </p>
        </div>
      </div>

      <DataTable<ActiveLeaseItem>
        data={pagedItems}
        columns={columns}
        loading={loading}
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="border border-gray-150 dark:border-gray-800 rounded-xl p-4 space-y-2.5 bg-white dark:bg-gray-900 shadow-sm">
                        {[
                          ...
                          { label: "Arrears", value: `LKR ${formatCurrency(selectedItem.arrearsAmount)}`, textClass: selectedItem.arrearsAmount > 0 ? "text-red-650 font-extrabold animate-pulse" : "" },
                          ...
                          <table className="w-full text-left border-collapse">
                            ...
                            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                              {detailedApp.installments.map((inst: InstallmentItem) => (
                                <tr key={inst.id} className="text-xs hover:bg-gray-50/50 dark:hover:bg-gray-850/50">
